...
# حل مشكلة الـ Logger التي ظهرت في الصورة الأخيرة
...
def get_content(app):
    prompt = f"JSON ONLY: {{\"post\": \"viral post for {app['name']}\", \"script\": \"10 sec script\"}}"
...
def make_video(app_name, script):
    # تحميل فيديو خلفية من Pexels
    headers = {"Authorization": PEXELS_API_KEY}
    v_url = requests.get("https://api.pexels.com/videos/search?query=technology&per_page=1", headers=headers).json()['videos'][0]['video_files'][0]['link']
    ...
    # مونتاج الفيديو الأساسي (بدون TextClip لتفادي خطأ ImageMagick)
    # إذا كنت تريد نصاً، سنعتمد على أنظمة الـ Action لتثبيته، لكن هذا الكود "الأكثر أماناً"
    clip = VideoFileClip("v.mp4").subclip(0, audio.duration).resize(height=1920).crop(x_center=540, width=1080)
