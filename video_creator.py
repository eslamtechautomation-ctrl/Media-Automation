...
# منع تهنيج الأكشن بسبب كثرة الـ Logs
proglog.default_bar_logger = lambda *args, **kwargs: None

# جلب المفاتيح من الـ Secrets اللي في الصورة
...
# قائمة تطبيقاتك الـ 20 لضمان الترويج المستمر
APPS = [
...
def get_next_app():
    """اختيار التطبيق التالي وحفظ الترتيب في ملف"""
...
def get_content(app):
    """توليد المحتوى مع معالجة ذكية للـ JSON"""
    prompt = f"Return ONLY JSON: {{\"post\": \"viral post for {app['name']}\", \"script\": \"short video script\"}}"
    try:
        ...
        if match:
            ...
            # تفادي الـ KeyError عن طريق توفير قيم افتراضية
            p = data.get('post') or data.get('facebook_post') or f"Check out {app['name']}!"
            s = data.get('script') or data.get('video_script') or f"Download {app['name']} now."
            return {"post": p, "script": s}
    except: pass
    return {"post": f"Top App: {app['name']}", "script": f"Download {app['name']} today."}

def make_video(app_name, script):
    """صناعة الفيديو وحل مشكلة ImageMagick"""
    # جلب فيديو خلفية
    ...
    # المونتاج (Vertical 9:16)
    clip = VideoFileClip("v.mp4").subclip(0, min(audio.duration, 15)).resize(height=1920).crop(x_center=540, width=1080)
    
    # محاولة إضافة نص (وعدم الفشل لو تعذر ذلك)
    try:
...
if __name__ == "__main__":
    ...
    info = get_content(current_app)
    video = make_video(current_app['name'], info['script'])
